...
const CouponManager = ({ isAdmin }) => {
  ...
  const loadCoupons = async () => {
    const data = await CouponService.getAll(); // Lee de la nube
    setCoupons(data);
  };
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    // Genera una imagen corporativa automática si no se pone una
    const finalImg = `https://placehold.co/600x400/161616/bf522b?text=${code}`;
    await CouponService.create(code, discount, finalImg); // Guarda en la nube
    loadCoupons();
    handleCloseModal();
  };
  ...
  // El alumno solo ve los activos, el admin ve todos
          ...
          displayedCoupons.map((c) => (
            <Card key={c.id} className={`relative overflow-hidden transition-all duration-300 ${!c.active ? 'opacity-50 grayscale' : 'hover:scale-[1.02]'}`}>
              ...
              <div className="p-6 text-center bg-gradient-to-br from-gray-50 to-white">
                <div className="font-mono font-extrabold text-[#bf522b] text-2xl tracking-wider bg-white py-2 px-4 rounded-xl border-2 border-dashed border-[#bf522b]/30 inline-block mb-3 shadow-sm">
                  {c.code}
                </div>
                <div className="text-5xl font-black text-[#161616] my-4">-{c.discount}%</div>
                
                {!isAdmin && c.active && (
                  <p className="text-sm text-green-600 font-bold bg-green-50 py-2 px-3 rounded-lg border border-green-100 mt-2">
                ...
                {isAdmin && (
                  <div className="mt-6 flex gap-2 justify-center border-t pt-4">
                    <button onClick={() => handleToggleStatus(c.id)} className="px-4 py-2 text-xs font-bold border rounded-lg hover:bg-gray-50 transition">
                      {c.active ? 'Desactivar' : 'Activar'}
                    </button>
                    <button onClick={() => handleDelete(c.id)} className="px-4 py-2 text-xs font-bold bg-red-50 text-red-600 border border-red-200 rounded-lg hover:bg-red-100 transition">
                      🗑 Eliminar
                    </button>
                  </div>
                )}
