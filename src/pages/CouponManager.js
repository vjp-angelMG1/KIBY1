      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {coupons.map((c) => (
          <Card key={c.id} className={`relative overflow-hidden transition-colors duration-300 ${!c.active ? 'opacity-50 grayscale' : ''}`}>
            ...
            <img src={c.img} className="w-full h-32 object-cover" alt="Coupon" />
            <div className="p-4 text-center">
              ...
              <div className="mt-4 flex gap-2 justify-center">
                <button onClick={() => handleToggleStatus(c.id)} className="px-3 py-1 text-xs border rounded hover:bg-gray-50" title={c.active ? 'Desactivar' : 'Activar'}>
                  {c.active ? 'ON' : 'OFF'}
                </button>
                <button onClick={() => handleDelete(c.id)} className="px-3 py-1 text-xs bg-red-50 text-red-600 border border-red-200 rounded hover:bg-red-100" title="Eliminar">🗑</button>
              </div>
            </div>
          </Card>
        ))}
