...
const AdminDashboard = ({ goToStore }) => {
  ...
  const [stats, setStats] = useState({ totalModules: 0, topModules: [] });
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      setStats({ totalModules: modules.length, topModules });
    };
    fetchStats();
  }, []);
              ...
              {monthlySales.map((data, index) => (
                <div key={index} className="flex flex-col items-center flex-1 h-full justify-end group">
                  <div className="relative w-full bg-gray-100 rounded-t-lg overflow-hidden" style={{ height: '100%' }}>
                    <div className="absolute bottom-0 w-full bg-[#bf522b] rounded-t-lg transition-all duration-500 ease-out group-hover:bg-[#a3441f]" style={{ height: `${(data.sales / maxSales) * 100}%` }}></div>
                  </div>
              ...
              {stats.topModules.length === 0 ? (
                ...
                stats.topModules.map((m, index) => (
                  <div key={m.id} className="flex items-center gap-3 bg-gray-50 p-3 rounded-lg">
                    <div className="w-8 h-8 bg-[#bf522b] text-white rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0">{index + 1}</div>
        ...
        <div className="lg:col-span-2">
          ...
          {myModules.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
              {myModules.map(m => (
                <div key={m.id} className="flex items-center bg-white p-3 rounded-lg border border-gray-100 shadow-sm">
